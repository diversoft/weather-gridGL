$(document).ready(function() {	
		...
		contents: [
			{typ: "div",
			 ...
			 text: "<span class='varname'>Issued "+date+" "+months[month]+" 2015</span><br>"+
				   ...
				   "<span class='varname'>Temperature, RH, Wind Speed, Precipitation</span><br><br>"+
				   "<span class='varname'>To move forward:</span><br>"+
	...
	function updateColor(direction) {
		...
		$.ajax({
			...
			success: function(dat) {
				if (showVar === 'prec') {
					console.log('Success with ut_data-'+pfcst+'-'+showVar+'.json');
				} else {
					console.log('Success with ut_data-'+forecast+'-'+showVar+'.json');
				}
				data = dat.a;
				data.map(function(d,i) {
					var colors = Colors2Get[showVar][d];
