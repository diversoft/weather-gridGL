...
hei = 1377
wid = 2145
latfile = open('lats.pkl')
lonfile = open('lons.pkl')
	...
	if outVar is not 'prec':
		while j<=fcs:
			...
			if outVar in ['rh','temp']:
				cmd = 'wgrib2 ds.temp.bin -match "TMP:surface:'+str(hour)+' hour fcst" -end -inv /dev/null -no_header -bin -'
				...
				if outVar is 'rh':
					tempDat = dat
					cmd = 'wgrib2 ds.td.bin -match "DPT:surface:'+str(hour)+' hour fcst" -end -inv /dev/null -no_header -bin -'
					p = Popen(cmd, shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
					output = p.stdout.read()
				...
				cmd = 'wgrib2 ds.wspd.bin -match "WIND:surface:'+str(hour)+' hour fcst" -end -inv /dev/null -no_header -bin -'
				p = Popen(cmd, shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
				output = p.stdout.read()
				dat = np.frombuffer(output[-wid*hei*4::],dtype=np.float32).reshape(hei,wid)
				dat = (dat*1.852)
			...
			if j<1:
				latsout = open('./east-data/east_lats.json','w')
				lonsout = open('./east-data/east_lons.json','w')
				latsout.write('var lats=[')
				lonsout.write('var lons=[')
				rlats = lats[656:1013,1625:1950]*100
				rlons = lons[656:1013,1625:1950]*100-36000
				#latsout.write(','.join(map(str,list(np.array(lats[463:1028,167:854]*100,dtype=np.int).flatten()))))
				#lonsout.write(','.join(map(str,list(np.array(lons[463:1028,167:854]*100,dtype=np.int).flatten()))))
			...
			datout = open('./east-data/east_data-'+str(j)+'-'+outVar+'.json','w')
		...
		while j<=pfcs:
			tm = time.time()
			hour = phours[j]
			cmd = 'wgrib2 ds.qpf.bin -match "APCP:surface:'+hour+' hour acc fcst" -end -inv /dev/null -no_header -bin -'
			#print cmd
			p = Popen(cmd, shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
			output = p.stdout.read()
			dat = np.frombuffer(output[-wid*hei*4::],dtype=np.float32).reshape(hei,wid)
