...
for outVar in outVars:
	...
	if outVar is not 'prec':
		while j<=fcs:
			...
			if j<1:
				latsout = open('./simple-data/smpl_lats.json','w')
				lonsout = open('./simple-data/smpl_lons.json','w')
				latsout.write('var lats=[')
				lonsout.write('var lons=[')
				rlats = lats[769:943,477:533]*100
				rlons = lons[769:943,477:533]*100-36000
			...
			datout = open('./simple-data/simple-'+str(j)+'-'+outVar+'.json','w')
			print 'Writing ut_data-'+str(j)+'-'+outVar+'.json'
			rdat = dat[769:943,477:533]
			if (outVar is 'rh'):
				bdat = dewDat[769:943,477:533]
				rdat[bdat>1e19] = 1e19
